...
수동 작업과 관리 오버헤드를 제거하고 즉시 자연스러운 번역을 제공합니다. Replexica 로컬라이제이션 엔진은 제품 컨텍스트를 이해하여 60개 이상의 언어에서 원어민이 기대하는 수준의 완벽한 번역을 만들어냅니다. 결과적으로 팀은 최첨단 품질의 로컬라이제이션을 100배 더 빠르게 수행하여 전 세계의 더 많은 고객에게 기능을 제공할 수 있습니다.
...
이 리포지토리에는 새로운 실험적 기능인 JS/React 로컬라이제이션 컴파일러도 포함되어 있습니다.

개발 팀이 **번역 파일로 문자열을 추출하지 않고도** 프론트엔드 로컬라이제이션을 수행할 수 있습니다. 단 한 줄의 코드로 다국어 프론트엔드를 구현할 수 있습니다. 빌드 타임에 작동하며 추상 구문 트리(AST) 조작과 코드 생성을 활용합니다.
